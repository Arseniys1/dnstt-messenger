...
## C0 — Проверка переводов

Мессенджер поддерживает 7 языков: английский, китайский, фарси, русский, арабский, турецкий и вьетнамский.

Для проверки полноты переводов используй скрипт валидации:
...
Или через npm (из папки electron-client):
...
Скрипт проверит:
- Все ли ключи переведены на каждый язык
- Нет ли лишних ключей
- Процент покрытия переводов (должен быть ≥95%)
